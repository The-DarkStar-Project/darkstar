...
from tools.bruteforce.integration import process_scan_results_with_hydra, get_hydra_protocol, process_bruteforce_results
...
class TestIntegration:
    ...
    @pytest.mark.asyncio
    async def test_process_scan_results_with_hydra_no_services(self, mocker: MockerFixture):
    ...
    @pytest.mark.asyncio
    async def test_process_scan_results_with_hydra_no_supported_services(self, mocker: MockerFixture):
        ...
        assert result == {}
        mock_logger.info.assert_called_with("No supported services found for bruteforcing")
    ...
    async def test_process_scan_results_with_hydra_success(self, mocker: MockerFixture):
        ...
        async def mock_run_hydra_attack_side_effect(sem, hydra, ip, protocol, port, timeout):
            if protocol == "ftp":
                return mock_attack_result_1
            else:
                return mock_attack_result_2
        
        mock_run_hydra_attack = mocker.patch("tools.bruteforce.integration.run_hydra_attack", side_effect=mock_run_hydra_attack_side_effect)
        ...
        result = await process_scan_results_with_hydra(scan_results, org_name="test_org")
        
        # Verify output directory creation
        mock_makedirs.assert_called_once_with("scan_results/test_org/bruteforce", exist_ok=True)
        
        # Verify HydraAttack was instantiated with correct output directory
        mock_hydra_class.assert_called_once_with(output_dir="scan_results/test_org/bruteforce")
        ...
        assert ftp_result["credentials"] == [{"username": "admin", "password": "123456"}]
        ...
        # Verify logging
        mock_logger.info.assert_any_call("Found supported service for bruteforcing: 192.168.1.1:21 - ftp -> ftp")
        mock_logger.info.assert_any_call("Found supported service for bruteforcing: 192.168.1.1:22 - ssh -> ssh")
        mock_logger.info.assert_any_call("Starting 2 Hydra attacks on supported services")

    @pytest.mark.asyncio
    async def test_process_scan_results_with_hydra_with_exceptions(self, mocker: MockerFixture):
    ...
    @pytest.mark.asyncio
    async def test_process_scan_results_with_hydra_multiple_ips(self, mocker: MockerFixture):
    ...
    @pytest.mark.asyncio
    async def test_process_scan_results_with_hydra_custom_parameters(self, mocker: MockerFixture):
        ...
        mock_makedirs.assert_called_once_with("scan_results/custom_org/bruteforce", exist_ok=True)
        mock_hydra_class.assert_called_once_with(output_dir="scan_results/custom_org/bruteforce")
    ...
    def test_process_bruteforce_results_empty(self, mocker: MockerFixture):
        ...
        mock_logger.info.assert_any_call(f"{Fore.YELLOW}[!] No bruteforce results available.{Style.RESET_ALL}")
    ...
    def test_process_bruteforce_results_no_credentials_found(self, mocker: MockerFixture):
    ...
    def test_process_bruteforce_results_credentials_found_username_password(self, mocker: MockerFixture):
        ...
        expected_result = {
            ...
            "credentials_by_host": {
                "192.168.1.1": [
                    {"username": "admin", "password": "123456", "port": 21, "protocol": "ftp"},
                    {"username": "user", "password": "password", "port": 21, "protocol": "ftp"}
                ],
                "192.168.1.2": [
                    {"username": "root", "password": "toor", "port": 22, "protocol": "ssh"}
                ]
            }
        }
        assert result == expected_result
    ...
    def test_process_bruteforce_results_snmp_community_strings(self, mocker: MockerFixture):
    ...
    def test_process_bruteforce_results_mixed_success_failure(self, mocker: MockerFixture):
    ...
    def test_process_bruteforce_results_success_but_empty_credentials(self, mocker: MockerFixture):
    ...
    def test_process_bruteforce_results_malformed_credentials(self, mocker: MockerFixture):
